...
if ($_POST['option']=='code'){
    ...
    $query = "select * from stocks where Code='" . $_POST['searchtext'] ."'";
    $data = mysqli_query($dbc, $query);

    if (mysqli_num_rows($data)==1){
        //the stock exists
        $row = mysqli_fetch_array($data);
        header('Location: http://localhost/guba/webs/stockpage.php?code='.$row['Code']);

    }else{
        echo "<script>alert('请输入合法代码')</script>";
    }
}
?>
...
?>
    <div class="panel panel-default">
        ...
        <table class="table table-striped table-hover">
            ...
            $query = "select * from stocks where `Name` like '%".$_POST['searchtext']."%'";
            $data = mysqli_query($dbc, $query);
